...
import { getActiveNotes } from "../api/notesAPI";
    ...
    return (
        <>
            <div className="bg-slate-200 px-24 min-h-screen pt-24 dark:bg-slate-800 dark:text-slate-100 transition-all">
                <header>
                    <h1 className="text-3xl font-semibold mb-4">{str[locale].homeTitle}</h1>
            ...
            <Link to='/notes/new' className="fixed top-[85%] left-[90%]"><button className="p-6 bg-slate-500 rounded-full shadow-md"><FaPlus className="text-white" /></button></Link>
        </>
    )
}
